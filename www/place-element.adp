...
<multiple name=element_multi>
    ...
    <if @element_multi:rowcount gt 1>
      <if @element_multi.rownum@ gt 1>
        (<a href="element-swap?portal_id=@portal_id@&element_id=@element_multi.element_id@&region=@region@&sort_key=@element_multi.sort_key@&direction=up">up</a>)
      </if>
      <if @element_multi:rowcount@ gt @element_multi.rownum@>
        (<a href="element-swap?portal_id=@portal_id@&element_id=@element_multi.element_id@&region=@region@&sort_key=@element_multi.sort_key@&direction=down">down</a>)
      </if>
    </if>
    <br>
</multiple>
